...
static uint64_t stableTicks=0, actionAttempts=0, actionVerified=0;
static bool awaitingVerify=false; static uint64_t preActionHash=0; static int verifyTicks=0;
static HWND hAutoState=nullptr,hAutoStats=nullptr;
...
uint64_t hashImg(const Img&i){
    // 32x18 perceptual luminance fingerprint. Much less sensitive to tiny animations than raw FNV sampling.
    uint64_t h=1469598103934665603ULL;
    const int gx=32, gy=18;
    for(int y=0;y<gy;y++) for(int x=0;x<gx;x++){
        int px=(x*i.w)/gx, py=(y*i.h)/gy;
        size_t n=((size_t)py*i.w+px)*4;
        unsigned b=i.px[n], g=i.px[n+1], r=i.px[n+2];
...
void automationTick(uint64_t hh){
    if(emergency || !running || controlLevel!=100 || !gFM){
        awaitingVerify=false; verifyTicks=0; stableTicks=0;
        if(hAutoState) setText(hAutoState, controlLevel==100?L"ARMED / WAITING":L"LOCKED");
        return;
    }
    if(awaitingVerify){
        verifyTicks++;
        if(hh!=preActionHash){ actionVerified++; awaitingVerify=false; verifyTicks=0; stableTicks=0; setText(hAutoState,L"VERIFIED • OBSERVING"); logLine(L"Autonomous CONTINUE verified"); }
        else if(verifyTicks>=8){ awaitingVerify=false; verifyTicks=0; stableTicks=0; setText(hAutoState,L"NO RESPONSE • SAFE RETRY"); logLine(L"Autonomous CONTINUE produced no visible response"); }
    } else {
        stableTicks++;
        if(stableTicks>=12){ // ~6 seconds of stable observation before an action
            preActionHash=hh; actionAttempts++;
            if(sendContinueKey()){ awaitingVerify=true; verifyTicks=0; setText(hAutoState,L"ACTING • VERIFYING"); logLine(L"Autonomous CONTINUE sent"); }
            else { stableTicks=0; setText(hAutoState,L"ACTION BLOCKED"); }
        }
    }
    if(hAutoStats) setText(hAutoStats,L"Attempts "+std::to_wstring(actionAttempts)+L"  •  Verified "+std::to_wstring(actionVerified));
}
